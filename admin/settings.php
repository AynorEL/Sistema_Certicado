...
if (isset($_POST['form1'])) {
    ...
    if ($valid == 1) {
        ...
        // Generando nombre único para la nueva imagen
        $final_name = 'logo_' . time() . '.' . $ext;
        
        // Moviendo la nueva imagen
...
if (isset($_POST['form2'])) {
    ...
    if ($valid == 1) {
        // Obteniendo el favicon existente
        $statement = $pdo->prepare("SELECT * FROM configuraciones WHERE id=1");
        $statement->execute();
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);
        foreach ($result as $row) {
            $favicon = $row['favicon'];
            // Eliminando el favicon existente si existe
            if ($favicon != '') {
